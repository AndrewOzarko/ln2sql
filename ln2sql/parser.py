        ...
        for i in range(0, len(input_word_list)):
            for table_name in self.database_dico:
                if (input_word_list[i] == table_name) or (
                            self.database_object.get_table_by_name(table_name).equivalences is not None and
                ...
                columns = self.database_object.get_table_by_name(table_name).get_columns()
                for column in columns:
                    if (input_word_list[i] == column.name) or (column.equivalences is not None and input_word_list[i] in column.equivalences):
